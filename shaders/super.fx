...
shared float4 fogParams; //(x=fogStart,y=fogEnd,z=(fogEnd-fogStart))
...
float4 ps_mesh(in MESH_OUTPUT In) : COLOR0
{
	...
	float specularInt;
#ifdef SPECULAR_MAP
	float4 spec_tex = tex2D(samplerSpecular, In.tex);
	specularInt = spec_tex.r + (1.f - spec_tex.a) * specularIntensity;
#else
	specularInt = specularIntensity;
#endif
