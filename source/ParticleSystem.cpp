...
void ParticleEmitter::Load(FileReader& f, int version)
{
	...
	else
	{
		...
		ParticleEffect* oldEffect = new ParticleEffect;
		float oldAlpha, oldSize;
		int opSize, opAlpha;
		oldEffect->tex = app::resMgr->Get<Texture>(f.ReadString1());
		f >> oldEffect->emissionInterval;
		f >> life;
		f >> oldEffect->particleLife;
		f >> oldAlpha;
		f >> oldSize;
		f >> emissions;
		f >> oldEffect->spawn;
		f >> oldEffect->maxParticles;
		f >> oldEffect->mode;
		f >> pos;
		f >> oldEffect->speedMin;
		f >> oldEffect->speedMax;
		f >> oldEffect->posMin;
		f >> oldEffect->posMax;
		f >> opSize;
		f >> opAlpha;
